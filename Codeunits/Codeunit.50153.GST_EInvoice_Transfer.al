codeunit 50153 E_Invoice_TransferShipments
{
    trigger OnRun()
    begin
    ...
    begin
        ...
        ValDtls.Add('TotInvVal', TotInvVal);
        //JsonWriter.WriteValue(0);//Naveen
    ...
    begin
        ...
        ELSE BEGIN
            ...
            if EinvoiceSetup."Enable Log" then begin
                ...
                LogEinvoice.SetRange("Document No.", TransferShipHeader."No.");
                if LogEinvoice.FindFirst() then begin
                    ...
                    LogEinvoice."Document No." := TransferShipHeader."No.";
    ...
    procedure Call_Ewaybill_API(recAuthData: Record "GST E-Invoice(Auth Data)"; JsonString: text; TransferShipHeader: record "Transfer Shipment Header"; IsEwayBill: Boolean; ISEWayCancel: Boolean)
    var
    ...
    begin
        ...
        if Httpclient.Post(PostUrl, HttpContent, httpresponse) then begin
            ...
            signedData := ParseResponse_IRN_ENCRYPT(responsetxt, false, TransferShipHeader, IsEwayBill, ISEWayCancel);
            ...
            File.CREATEOUTSTREAM(Outstr);
            Outstr.WRITETEXT(decryptedIRNResponse);*/
            ParseResponse_IRN_DECRYPT(decryptedIRNResponse, false, TransferShipHeader, IsEwayBill, ISEWayCancel);
    ...
    var
        ...
        EinvoiceSetup: Record "GST E-Invoice(Auth Data)";
        errorid: Text;
        encode: Codeunit "Base64 Convert";
        info: text;
    begin
        ...
        if ResponseJson.Get('status', Jtoken) then//eway bill response WO irn
            if Jtoken.AsValue().AsText() = '0' then begin
                if EinvoiceSetup."Enable Log" then begin
                    LogEinvoice.Reset();
                    LogEinvoice.SetRange("Document Type", LogEinvoice."Document Type"::Invoice);
                ...
                if ResponseJson.Get('error', Jtoken) then begin
                    errorid := Jtoken.AsValue().AsText();
                    errorid := encode.FromBase64(errorid);
                end;
                if ResponseJson.Get('info', Jtoken) then begin
                    info := Jtoken.AsValue().AsText();
                    info := encode.FromBase64(info);
                end;
                if IsEwayBill then
                    Error('Error in E-Way Bill generation : %1\error:%2,info:%3', TextResponse, errorid, info)
                else
                    if IsEwayCancel then
                        Error('Error in E-Way Bill cancellation : %1', TextResponse)
                    else
    ...
    procedure Call_EwaybillWOIRN_API(recAuthData: Record "GST E-Invoice(Auth Data)"; JsonString: text; TransferShipHeader: record "Transfer Shipment Header"; IsEwayBill: Boolean; ISEWayCancel: Boolean)
    var
    ...
    begin
        ...
        if DataJson.get('ewayBillDate', jtoken) then
            if not jtoken.AsValue().IsNull() then
                txtEWBDt := jtoken.AsValue().AsText();
        ...
        if DataJson.get('validUpto', jtoken) then
            if not jtoken.AsValue().IsNull() then
                txtEWBValid := jtoken.AsValue().AsText();
        ...
        if DataJson.get('CancelDate', jtoken) then
            if not jtoken.AsValue().IsNull() then begin
                txtCancelDate := jtoken.AsValue().AsText();
        ...
        IF recTransferShipHeader.find('-') THEN begin
            if IsEwayBill then begin
                toloc.Get(recTransferShipHeader."Transfer-from Code");
                froloc.get(recTransferShipHeader."Transfer-to Code");
                if toloc."GST Registration No." = froloc."GST Registration No." then
                    CU_EWayBill.UpdateHeaderWOIRN(txtEWBDt, txtEWBNum, txtEWBValid, TransferShipHeader)//230622
                else
        ...
        IF RecRef1.FINDFIRST THEN BEGIN
            ...
            if EinvoiceSetup."Enable Log" then begin
                ...
                if Not LogEinvoice.FindFirst() then begin
                    ...
                    LogEinvoice."Document Type" := LogEinvoice."Document Type"::Invoice;
                    LogEinvoice."Document No." := TransferShipHeader."No.";
                    LogEinvoice."Invoice Reference Number" := TransferShipHeader."IRN Hash";
                    LogEinvoice."Acknowledgment Date" := format(TransferShipHeader."Acknowledgement Date");
                    LogEinvoice.Acknowledgment_number := TransferShipHeader."Acknowledgement No.";
